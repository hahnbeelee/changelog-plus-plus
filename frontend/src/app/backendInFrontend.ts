...
async function getRepoDiff(
  ...
  try {
    ...
    const diffs = commits.flatMap((commit) =>
      commit.files.map(
        (file) =>
          `Author: ${commit.author.login}. File: ${file.filename}\n${file.patch}`,
      ),
    );
  ...
  try {
    const queryPayload = {
      messages: [
        {
          content:
            `Generate a concise changelog based on the following diffs. 
                              Make sure to group the changes by each developer with their profile picture in markdown: ${diffs.join(
                                "\n\n",
                              )} Here is a map of all the developer profile pictures:` +
            profilePics,
          role: "user",
        },
      ],
...
export async function generateChangelogForGreptileDocs(
  ...
  try {
    ...
    const changelog = await generateChangelog(
      repoDiffResult.diffs,
      repoDiffResult.profilePicMap,
      repo,
      owner,
    );
    return changelog;
